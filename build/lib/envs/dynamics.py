...
def hx_aer_astropy(
    x_gcrs, time, observer_lla=None, trans_matrix=None, observer_itrs=None
):
    """
    desc: measurement function - convert state into a measurement where measurements are [azimuth, elevation]
    :param x_gcrs:
    ...
    if observer_itrs is None:
        obs = EarthLocation.from_geodetic(
            lat=observer_lla[0] * u.rad,
            lon=observer_lla[1] * u.rad,
            height=observer_lla[2] * u.m,
        )
    else:
        obs = EarthLocation.from_geocentric(
            x=observer_itrs[0] * u.m, y=observer_itrs[1] * u.m, z=observer_itrs[2] * u.m
        )
...
def residual_z_aer(a, b):
    ...
    c = np.empty(a.shape)
    c[0] = np.arctan2(
        np.sin(a[0] - b[0]), np.cos(a[0] - b[0])
    )  # azimuth: input range (0, 2pi), output range (-pi, pi)
...
def mean_z_aer_depro(sigmas, Wm):
    # deprecated
    z = np.zeros(3)
    sum_sin_az, sum_cos_az, sum_sin_el, sum_cos_el = 0.0, 0.0, 0.0, 0.0
...
def mean_z_enu(sigmas, Wm):
    ...
    """
    enu_mean = np.average(
        np.array([pm.aer2enu(*sigma, deg=False) for sigma in sigmas]),
        axis=0,
        weights=Wm,
    )
...
def init_state_vec(orbits="Default", random_state=np.random.RandomState()):
    ...
    raan = np.radians(
        random_state.uniform(0, 360)
    )  # (rad) – Right ascension of the ascending node.
    argp = np.radians(
        random_state.uniform(low=0, high=360)
    )  # (rad) – Argument of the pericenter.
    nu = np.radians(random_state.uniform(0, 360))  # (rad) – True anomaly.

    if orbits[-1] == "LEO":
        while not exo_atmospheric:
            a = random_state.uniform(
                RE_eq + 300 * 1000, RE_eq + 2000 * 1000
            )  # (m) – Semi-major axis.
            ecc = random_state.uniform(0, 0.25)  # (Unit-less) – Eccentricity.
            b = a * np.sqrt(1 - ecc**2)
            if b > RE_eq + 300 * 1000:
    ...
    if orbits[-1] == "MEO":
        while not exo_atmospheric:
            a = random_state.uniform(
                RE_eq + 2000 * 1000, RE_eq + 35786 * 1000
            )  # (m) – Semi-major axis.
            ecc = random_state.uniform(0, 0.25)  # (Unit-less) – Eccentricity.
            b = a * np.sqrt(1 - ecc**2)
            if b > RE_eq + 300 * 1000:
    ...
    if orbits[-1] == "GEO":
        ...
        ecc = stationary * random_state.uniform(0, 0.25)  # (Unit-less) – Eccentricity.
