...
def errors(states, filters_x, filters_diag_P):
    ...
    sigma_position = np.zeros(
        shape=(n, m)
    )  # magnitude of position uncertainty (meters)
    sigma_velocity = np.zeros(
        shape=(n, m)
    )  # magnitude of velocity uncertainty (meters)
...
def error(states, obs):
    ...
    :return: magnitude of position - velocity uncertainty & delta(mean- true) position and velocity
    """
    delta_position = dist3d(
        obs[:, :3], states[:, :3]
    )  # mean position - true position (meters)
    delta_velocity = dist3d(
        obs[:, 3:6], states[:, 3:]
    )  # mean velocity - true velocity (meters)
...
def plot_delta_sigma(
    ...
    if ylim == "max":
        ...
        pos_lims_min = 10 ** np.floor(
            np.log10(np.min([np.nanmin([delta_pos, sigma_pos])]))
        )
    ...
    if ylim == "max":
        ...
        vel_lims_min = 10 ** np.floor(
            np.log10(np.min([np.nanmin([delta_vel, sigma_vel])]))
        )
...
def plot_performance(
    rewards, dt, t_0, names=None, sigma=1.5, style=None, yscale="linear"
):
...
def plot_nees(
    ...
    if axis is None:
        ...
        if alpha is not None:
            ...
            plt.hlines(
                y=cr_points[1], xmin=x[0], xmax=x[-1], color="red", linestyle="--"
            )
            label = (
                "Point Confidence Region, alpha = "
                + str(alpha)
                + "; "
                + str(np.round(points_contained * 100, 2))
                + "% contained"
            )
            legend_elements.append(
                mpl.lines.Line2D(
                    [0], [0], color="red", lw=2, linestyle="--", label=label
                )
            )
        ...
        if n_mov_ave is not None:
            ...
            if alpha is not None:
                ...
                plt.hlines(
                    y=cr_bar[1], xmin=x[0], xmax=x[-1], color="blue", linestyle="--"
                )
                legend_elements.append(
                    mpl.lines.Line2D(
                        [0],
    ...
    if axis == 0:
        ...
        legend_elements = [
            mpl.lines.Line2D([0], [0], color="blue", lw=2, label="ANEES by RSO"),
            mpl.lines.Line2D(
                [0],
                [0],
                color="black",
...
def moving_average_plot(
    ...
    plt.plot(
        ...
        linestyle="-",
        linewidth=2,
        label=str(n) + " step moving average of " + llabel,
    )
    if alpha is not None:
        contained = np.mean((x_bar > cr[0]) * (x_bar < cr[1]))
        plt.plot(
            np.repeat(cr[1], len(x_bar)),
            color="blue",
            linestyle="--",
            linewidth=2,
            ...
            + str(alpha)
            + "; "
            + str(np.round(contained * 100, 2))
            + "% contained",
        )
        plt.plot(
            np.repeat(cr[0], len(x_bar)), color="blue", linestyle="--", linewidth=2
        )
        points_contained = np.mean(
            (x[~np.isnan(x)] > cr_points[0]) * (x[~np.isnan(x)] < cr_points[1])
        )
        plt.plot(
            np.repeat(cr_points[1], len(x_bar)),
            color="red",
            linestyle="--",
            linewidth=2,
            label="Point confidence region, alpha = "
            + str(alpha)
            + "; "
            + str(np.round(points_contained * 100, 2))
            + "% contained",
        ...
        plt.plot(
            np.repeat(cr_points[0], len(x_bar)),
            color="red",
            linestyle="--",
            linewidth=2,
        )
...
def bound_plot(
    ...
    frac_sigma_bound = np.mean(
        (y_not_nan < st_dev_not_nan) * (y_not_nan > -st_dev_not_nan), axis=0
    )
    frac_two_sigma_bound = np.mean(
        (y_not_nan < 2 * st_dev_not_nan) * (y_not_nan > -2 * st_dev_not_nan), axis=0
    )
    ...
    for i in range(len(y[0])):
        ...
        axs[i].scatter(
            x=y_ID, y=y_not_nan[:, i], color="red", marker="+", label=ylabel[i]
        )
        axs[i].plot(
            y_ID,
            2 * st_dev_not_nan[:, i],
            ...
            linewidth=2,
            label="$2\sigma$ bound, "
            + str(np.round(frac_two_sigma_bound[i] * 100, 2))
            + "% contained",
        )
        axs[i].plot(
            y_ID,
            ...
            linewidth=2,
            label="$\sigma$ bound, "
            + str(np.round(frac_sigma_bound[i] * 100, 2))
            + "% contained",
        )
        axs[i].plot(
            y_ID, -st_dev_not_nan[:, i], color="blue", linestyle="--", linewidth=2
        )
        axs[i].plot(
            y_ID, -2 * st_dev_not_nan[:, i], color="blue", linestyle="-", linewidth=2
        )
        axs[i].legend()
...
def map_plot(x_filter, x_true, trans_matrix, observer):
    ...
    observer = (
        np.degrees(observer[0]),
        np.degrees(observer[1]),
        observer[2],
    )  # lat (deg), lon (deg), height (meters)
    ...
    observer = np.array([observer[1], observer[0]])
    ax.annotate(
        "Observation Station",
        xy=observer,
        xycoords="data",
