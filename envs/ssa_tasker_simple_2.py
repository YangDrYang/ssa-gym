...
from envs.transformations import (
    arcsec2rad,
    deg2rad,
    lla2ecef,
    gcrs2irts_matrix_a,
...
from envs.dynamics import (
    fx_xyz_farnocchia as fx,
    hx_aer_erfa as hx,
    mean_z_uvw as mean_z,
    residual_z_aer as residual_z,
)
from envs.dynamics import robust_cholesky
from envs.results import (
    observations as obs_fn,
    error,
    error_failed,
    plot_delta_sigma,
    plot_rewards,
...
from envs.results import (
    ...
    plot_orbit_vis,
    plot_regimes,
    reward_proportional_trinary_true,
)
...
class SSA_Tasker_Env(gym.Env):
    ...
    def __init__(self, config=env_config):
        ...
        self.runtime = {
            ...
            "step": 0,
            "step prep": 0,
            "propagate next true state": 0,
            "perform predictions": 0,
            "update with observation": 0,
            "Observations and Reward": 0,
            "filter_error": 0,
            "visible_objects": 0,
            "object_visibility": 0,
            "anees": 0,
        ...
        self.m = config[
            "rso_count"
        ]  # number of Resident Space Object (RSO) to include in the simulation
        self.obs_limit = np.radians(
            config["obs_limit"]
        )  # don't observe objects below this elevation [rad]
        ...
        self.obs_lla = np.array(config["observer"]) * [
            deg2rad,
            deg2rad,
            1,
        ]  # Observer coordinates - lat, lon, height (deg, deg, m)
        ...
        self.alpha, self.beta, self.kappa = (
            ...
            config["beta"],
            config["kappa"],
        )  # sigma point configuration parameters
        ...
        if config["P_0"] is None:
            self.P_0 = np.copy(
                np.diag(self.x_sigma**2)
            )  # Assumed covariance of the estimates at simulation start
        else:
            self.P_0 = np.copy(config["P_0"])
        if config["R"] is None:
            self.R = np.diag(
                self.z_sigma**2
            )  # Noise added to the filter during observation updates
        else:
            self.R = np.copy(config["R"])
        self.x_true = np.empty(
            shape=(self.n, self.m, x_dim)
        )  # true means for all objects at each time step
        self.x_filter = np.empty(
            shape=(self.n, self.m, x_dim)
        )  # filter means for all objects at each time step
        self.P_filter = np.empty(
            shape=(self.n, self.m, x_dim, x_dim)
        )  # covariances for all objects at each time step
        self.obs = np.empty(
            shape=(self.n, self.m, x_dim * 2)
        )  # observations for all objects at each time step
        self.time = [
            self.t_0 + (timedelta(seconds=self.dt) * i) for i in range(self.n)
        ]  # time for all time steps
        self.trans_matrix = gcrs2irts_matrix_b(
            self.time, self.eops
        )  # used for celestial to terrestrial
        self.z_noise = np.empty(
            shape=(self.n, self.m, z_dim)
        )  # array to contain the noise added to each observation
        self.z_true = np.empty(
            shape=(self.n, self.m, z_dim)
        )  # array to contain the true observations which are made
        self.y = np.empty(
            shape=(self.n, self.m, z_dim)
        )  # array to contain the innovation of each observation
        self.S = np.empty(
            shape=(self.n, self.m, z_dim, z_dim)
        )  # array to contain the innovation covariance
        self.x_noise = np.empty(
            shape=(self.m, x_dim)
        )  # array to contain the noise added to each RSO
        self.filters = []  # creates a list for ukfs

        """variables for environment performance"""
        self.delta_pos = np.empty(
            (self.n, self.m)
        )  # euclidean distance between true and filter mean position elements
        self.delta_vel = np.empty(
            (self.n, self.m)
        )  # euclidean distance between true and filter mean velocity elements
        self.sigma_pos = np.empty(
            (self.n, self.m)
        )  # euclidean magnitude of diagonal position elements of covariance
        self.sigma_vel = np.empty(
            (self.n, self.m)
        )  # euclidean magnitude of diagonal velocity elements of covariance
        ...
        self.actions = np.empty(
            self.n, dtype=int
        )  # prep variable for keeping track of all previous actions
        self.obs_taken = np.empty(
            self.n, dtype=bool
        )  # prep variable for keeping track of which obs were actually taken
        self.x_failed = np.array(
            [1e20, 1e20, 1e20, 1e12, 1e12, 1e12]
        )  # failed filter will be set to this value
        self.P_failed = np.diag(
            [1e20, 1e20, 1e20, 1e12, 1e12, 1e12]
        )  # failed filter will be set to this value
        self.nees = np.empty(
            (self.n, self.m)
        )  # used for normalized estimation error squared (NEES) and its average
        self.visibility = []  # used to store a log of visible objects at each time step
        self.sigmas_h = np.empty(
            (self.n, x_dim * 2 + 1, z_dim)
        )  # used to store sigmas points used in updates

        """Define Gym spaces"""
        self.action_space = gym.spaces.Discrete(
            self.m
        )  # the action is choosing which RSO to look at
        if self.obs_returned == "flatten":
            self.observation_space = gym.spaces.Box(
                low=np.tile(-np.inf, (self.m * 12)),  # flat 1d array for MLP
                high=np.tile(np.inf, (self.m * 12)),
                dtype=np.float64,
            )  # obs is x [6] and diag(P) [6] for each RSO [m]
        elif self.obs_returned == "aer":
            self.observation_space = gym.spaces.Box(
                low=np.tile(-np.inf, (self.m * 4)),  # flat 1d array for MLP
                high=np.tile(np.inf, (self.m * 4)),
                dtype=np.float64,
            )  # obs is aer [3] and trace(P) [1] for each RSO [m]
            self.observation = np.zeros(self.m * 4)
        else:
            self.observation_space = gym.spaces.Box(
                low=np.tile(-np.inf, (self.m, 12)),  # 2d array for readability
                high=np.tile(np.inf, (self.m, 12)),
                dtype=np.float64,
            )  # obs is x [6] and diag(P) [6] for each RSO [m]
    ...
    def reset(self):
        ...
        for j in range(self.m):
            # self.x_true[0][j] = self.orbits[
            #     self.np_random.randint(low=0, high=self.orbits.shape[0]), :
            # ]
            self.x_true[0][j] = self.orbits[
                self.np_random.integers(low=0, high=self.orbits.shape[0]), :
            ]
        ...
        self.obs[0] = obs_fn(
            self.x_filter[0], self.P_filter[0]
        )  # set initial observation based on x and P
        (
            self.delta_pos[0],
            self.delta_vel[0],
            self.sigma_pos[0],
    ...
    def step(self, a):
        ...
        step_s = time.time()
        s = time.time()
        assert self.action_space.contains(a), "%r (%s) invalid" % (
            a,
            type(a),
                ...
                try:
                    ...
                    if np.any(np.isnan(self.filters[j].x)):
                        self.filter_error(
                            object_id=j,
                            activity="predict",
                            error_type=", predict returned nan. ",
                        )
                except ValueError:
                    self.filter_error(
                        object_id=j, activity="predict", error_type=", ValueError. "
                    )
                except np.linalg.LinAlgError:
                    self.filter_error(
                        object_id=j, activity="predict", error_type=", LinAlgError. "
                    )
                except:
                    self.filter_error(
                        object_id=j, activity="predict", error_type=", Unknown. "
                    )
            self.x_filter[self.i, j] = np.copy(
                self.filters[j].x
            )  # update filter mean history
            self.P_filter[self.i, j] = np.copy(
                self.filters[j].P
            )  # update covariance mean history
        e = time.time()
        ...
        if (self.i % self.update_interval) == 0:
            if not (a in self.failed_filters_id):
                hx_kwargs = {
                    "trans_matrix": self.trans_matrix[self.i],
                ...
                if self.object_visible([a])[0]:
                    try:
                        self.filters[a].update(
                            self.z_true[self.i, a] + self.z_noise[self.i, a],
                            **hx_kwargs
                        )
                        ...
                        if np.any(np.isnan(self.filters[a].x)):
                            self.filter_error(
                                object_id=a,
                                activity="update",
                                error_type=", update returned nan. ",
                            )
                    except ValueError:
                        self.filter_error(
                            object_id=a, activity="update", error_type=", ValueError. "
                        )
                    except np.linalg.LinAlgError:
                        self.filter_error(
                            object_id=a, activity="update", error_type=", LinAlgError. "
                        )
                    except:
                        self.filter_error(
                            object_id=a, activity="update", error_type=", Unknown. "
                        )
                    self.x_filter[self.i, a] = np.copy(
                        self.filters[a].x
                    )  # update filter mean history
                    self.P_filter[self.i, a] = np.copy(
                        self.filters[a].P
                    )  # update covariance mean history
        e = time.time()
        ...
        (
            self.delta_pos[self.i],
            self.delta_vel[self.i],
            self.sigma_pos[self.i],
            self.sigma_vel[self.i],
        ) = tmp
        done = False
        if self.reward_type == "jones":
            ...
            self.rewards[self.i] = reward_proportional_trinary_true(
                self.delta_pos[self.i]
            )  # reward scaling for DRL
        ...
        if self.obs_returned == "flatten":
            ...
            return (
                self.observation,
                np.nan_to_num(
                    self.rewards[self.i], copy=False, nan=0.5, posinf=0.5, neginf=0.5
                ),
                done,
                {},
            )  # observations, reward, and done
            ...
            "".join(
                [
                    ...
                    str(
                        np.round(
                            error_failed(
                                state=self.x_true[self.i - 1, object_id],
                                x=self.x_filter[self.i - 1, object_id],
                                P=np.diag(self.P_filter[self.i - 1, object_id]),
                            ),
                            2,
                        )
                    ),
    ...
    def render(self, mode="live"):
        ...
        if mode == "live":
            if self.i <= self.n:
                ...
                orb = [
                    Orbit.from_vectors(
                        Earth, r=(x[:3] / 1000) * u.km, v=(x[3:] / 1000) * u.km / u.s
                    )
                    for x in self.x_true[self.i]
                ]
                orb1 = [
                    Orbit.from_vectors(
                        Earth, r=(x[:3] / 1000) * u.km, v=(x[3:] / 1000) * u.km / u.s
                    )
                    for x in self.x_filter[self.i]
                ]
    ...
    def object_visible(self, RSO_ID=[]):
        if not RSO_ID:
            ...
            return RSO_ID
        x_itrs = np.array(
            [self.trans_matrix[self.i] @ self.x_true[self.i, j, :3] for j in RSO_ID]
        )
    ...
    def object_visibility(self):
        ...
        x_itrs = np.array(
            [
                self.trans_matrix[self.i] @ self.x_true[self.i, j, :3]
                for j in range(self.m)
            ]
        )
        el = np.array([ecef2aer(self.obs_lla, x, self.obs_itrs)[1] for x in x_itrs])
    ...
    def anees(self):
        ...
        for i in range(self.n):
            for j in range(self.m):
                self.nees[i, j] = (
                    delta[i, j] @ np.linalg.inv(self.P_filter[i, j]) @ delta[i, j]
                )
    ...
    def plot_sigma_delta(
        ...
        if objects.shape[0] == 0:
            ...
            plot_delta_sigma(
                sigma_pos=self.sigma_pos[:, objects],
                sigma_vel=self.sigma_vel[:, objects],
                delta_pos=self.delta_pos[:, objects],
                delta_vel=self.delta_vel[:, objects],
                dt=self.dt,
                t_0=self.t_0,
                style=style,
                yscale=yscale,
    ...
    def plot_rewards(self, style=None, yscale="linear"):
        s = time.time()
        plot_rewards(
            rewards=self.rewards, dt=self.dt, t_0=self.t_0, style=style, yscale=yscale
        )
        e = time.time()
    ...
    def plot_actions(self, axis=0, title="default", save_path="default", display=True):
        ...
        if title == "default":
            title = (
                "Frequency of observation for RSO (ID), "
                + str(self.m)
                + " RSOs, seed = "
                + str(self.init_seed)
            )
        if save_path == "default":
            save_path = (
                str(self.m) + "RSO_" + str(self.init_seed) + "seed_action_plot.svg"
            )
        plot_histogram(
            self.actions[1:],
            bins="int",
    ...
    def plot_visibility(self, save_path=None, display=True):
        ...
        title = (
            "Visibility Plot (white = visible); elevation > "
            + str(np.round(np.degrees(self.obs_limit), 0))
            + " degrees"
        )
    ...
    def plot_regimes(self, save_path=None, display=True):
        ...
        coes = np.array(
            rv2coe(
                k=Earth.k.to_value(u.km**3 / u.s**2), r=x[:3] / 1000, v=x[3:] / 1000
            )
            for x in self.x_true[0]
        )
    ...
    def plot_NIS(self, save_path=None, display=True):
        ...
        xlabel = "Time Step"
        ylabel = (
            "$NIS = (z_{obs}^t-z_{pred}^t)(S^t)^{-1}(z_{obs}^t-z_{pred}^t)$ for i = [0, "
            + str(self.n)
            + "), j = [0, "
            + str(self.m)
            + ")"
    ...
    def plot_innovation_bounds(self, ID=None, save_path=None, display=True):
        if ID is None:
            ...
            st_dev = np.sqrt(
                [np.diag(self.S[i, self.actions[i]]) for i in range(1, self.n)]
            )
        else:
    ...
    def innovation_bounds(self):
        ...
        frac_sigma_bound = np.mean(
            (innovation < st_dev) * (innovation > -st_dev), axis=0
        )
        frac_two_sigma_bound = np.mean(
            (innovation < 2 * st_dev) * (innovation > -2 * st_dev), axis=0
        )
        data = np.round(np.stack((frac_sigma_bound, frac_two_sigma_bound)) * 100, 2)
        if self.obs_type == "xyz":
            df = pd.DataFrame(
                data,
                index=["Sigma", "Two Sigmas"],
                columns=["x (meters)", "y (meters)", "z (meters)"],
            )
        if self.obs_type == "aer":
            df = pd.DataFrame(
                data,
    ...
    def plot_map(self, objects=np.array([]), timesteps="All"):
        ...
        if objects.shape[0] == 0:
            ...
            x_filter = self.x_filter[
                timesteps[0] : timesteps[1], objects[0] : objects[1]
            ]
    ...
    def innovation(self):
        ...
        indexes_not = [
            np.where(((self.actions[1:] == i) == False) * (self.obs_taken[1:]))
            for i in RSO_ID
        ]
    ...
    def autocorrelation(self):
        ...
        for i in range(0, 3):
            autocorrelation.append(
                acf(x=innovation[:, i], missing="conservative", fft=False)
            )
        ...
        for j in range(len(innovations)):
            ...
            for i in range(0, 3):
                aa.append(
                    acf(x=innovations[j][:, i], missing="conservative", fft=False)
                )
    ...
    def plot_autocorrelation(self, RSO_ID=None, save_path=None, display=True):
        ...
        fig.suptitle(
            "Autocorrelation of the Innovation \n lines for overall, colored `x` for individual objects"
        )
        ci = 2 / np.sqrt(self.n)
        for i in range(0, 3):
            axs[i].bar(
                x=np.arange(0, len(autocorrelation[i])),
                height=autocorrelation[i],
                width=0.3,
            )
            ...
            for j in range(len(RSO_ID)):
                axs[i].scatter(
                    x=np.arange(0, len(autocorrelations[j][i])),
                    y=autocorrelations[j][i],
                    marker="x",
                )
    ...
    def fitness_test(self):
        ...
        for i in range(autocorr.shape[0]):
            autocorr_contained.append(
                np.mean(
                    (autocorr[i] < 2 / np.sqrt(self.n))
                    * (autocorr[i] > -2 / np.sqrt(self.n))
                )
            )
        ...
        rso_innovation_acorr_stat = [
            np.max(rso_innovation_acorr, axis=0),
            np.min(rso_innovation_acorr, axis=0),
        ]

        innovation_acorr_stats = np.round(
            np.vstack((overall_innovation_acorr, rso_innovation_acorr_stat)) * 100, 2
        )

        innovation_autocorrelation_test = pd.DataFrame(
            innovation_acorr_stats,
            index=[
                "Test 3a: Acorr overall 2σ",
        ...
        innovation_nonan = np.array(
            [
                innovation[~np.isnan(innovation[:, i]), i]
                for i in range(len(innovation[0]))
            ]
        ).T
        inn_st_dev_nonan = np.array(
            [
                inn_st_dev[~np.isnan(inn_st_dev[:, i]), i]
                for i in range(len(inn_st_dev[0]))
            ]
        ).T
        inn_frac_sigma_bound = np.mean(
            (innovation_nonan < inn_st_dev_nonan)
            * (innovation_nonan > -inn_st_dev_nonan),
            axis=0,
        )
        inn_frac_two_sigma_bound = np.mean(
            (innovation_nonan < 2 * inn_st_dev_nonan)
            * (innovation_nonan > -2 * inn_st_dev_nonan),
            axis=0,
        )
        inn_frac_bound = np.round(
            np.stack((inn_frac_sigma_bound, inn_frac_two_sigma_bound)) * 100, 2
        )
        innovation_bound_test = pd.DataFrame(
            inn_frac_bound,
            index=["Test 1a: Innovation 1σ bound", "Test 1b: Innovation 2σ bound"],
            columns=clabels,
        )
        ...
        normalized_innovation_squared_test = pd.DataFrame(
            np.round(points_contained * 100, 2),
            index=["Test 2: NIS χ2"],
            columns=["95% CI"],
        )
        ...
        print(self.nees)
        nees_points_contained = np.mean(
            (self.nees > nees_cr_points[0]) * (self.nees < nees_cr_points[1])
        )
        normalized_estimation_error_squared_test = pd.DataFrame(
            np.round(nees_points_contained * 100, 2),
    ...
    def innovation_dw_test(self):
        ...
        dw_innovation = np.round(
            np.sum(diff_innovation**2, axis=0) / np.sum(innovation**2, axis=0), 3
        )
        dw_innovations = []
        # Durbin-Watson statistic for min-max per obj
        for inn in innovations:
            diff_inn = np.diff(inn[0], 1, axis=0)
            dw_innovations.append(
                np.sum(diff_inn**2, axis=0) / np.sum(inn[0] ** 2, axis=0)
            )
        dw_innovations = np.array(dw_innovations)
        dw_innovations_stat = np.round(
            np.stack([np.min(dw_innovations, axis=0), np.max(dw_innovations, axis=0)]),
            3,
        )
        if self.obs_type == "xyz":
            clabels = ["x", "y", "z"]
            ...
            clabels = ["Azimuth", "Elevation", "Range"]
        dw_autocorr_test = pd.DataFrame(
            data=np.vstack([dw_innovation, dw_innovations_stat]),
            columns=clabels,
            index=[
                "Durbin-Watson Statistic for All Obs",
